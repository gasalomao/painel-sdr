...
import { Check, Sparkles, Wrench, ChevronDown } from "lucide-react";
 ...
 * Mostrado abaixo do campo `webhookUrl` na aba Info — accordion fechado
 * por padrão (a maioria dos usuários usa o "Sincronizar Agora" que faz
 * tudo automático, não precisa do passo-a-passo entulhando a UI).
 */
export function WebhookGuide({ webhookUrl }: { webhookUrl: string }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#00ffcc]/5 via-transparent to-purple-500/5 overflow-hidden">
      {/* Header — clicável pra expandir/colapsar */}
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full px-5 py-4 border-b border-white/5 bg-black/30 hover:bg-black/40 transition-colors text-left flex items-center gap-3"
        aria-expanded={open}
        aria-controls="webhook-guide-content"
      >
        <div className="p-2 rounded-lg bg-[#00ffcc]/10 border border-[#00ffcc]/20 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">
            {open
              ? "Passo a passo manual — use o Sincronizar Agora pra fazer automático."
              : "Clique pra ver o passo a passo manual (avançado). Geralmente não precisa."}
          </p>
        </div>
        <ChevronDown
          className={cn(
            "w-4 h-4 text-[#00ffcc] shrink-0 transition-transform",
            open && "rotate-180"
          )}
        />
      </button>

      {/* Conteúdo: só renderiza quando expandido — economiza pintura DOM
          quando colapsado (que é o estado padrão da maioria dos casos) */}
      {!open ? null : (
      <div id="webhook-guide-content" className="p-5 space-y-4">
        {/* 1 */}
