...
export function AutomationLogs({
  ...
  const fetchAll = useCallback(async (opts?: { silent?: boolean }) => {
    // silent=true: re-fetch de fundo (realtime/polling) — não pisca o
    // "Carregando histórico…". Só o fetch inicial mostra o loading.
  ...
  useEffect(() => {
    ...
    const debouncedFetch = () => {
      ...
      timer = setTimeout(() => fetchAll({ silent: true }), 600);
    };
    const ch = supabase
  ...
  // Fallback de polling — o painel NÃO pode depender só do realtime do
  // Supabase. Se `automation_logs` não estiver na publication
  // `supabase_realtime` (ou o canal cair), os eventos param de chegar e o
  // log congela no snapshot do mount. Enquanto a automação está ativa
  // (scraping ou com campanha/follow-up rodando), re-busca a cada 5s pra
  // garantir que o painel acompanha o que o scraper está fazendo de verdade.
  useEffect(() => {
    const active = scraping || !!campaignId || !!followupCampaignId;
    if (!active) return;
    const id = setInterval(() => fetchAll({ silent: true }), 5000);
    return () => clearInterval(id);
  }, [scraping, campaignId, followupCampaignId, fetchAll]);
