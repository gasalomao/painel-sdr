...
async function requireAdmin(req: NextRequest) {
  ...
  if (!ctx.isAdmin) {
    return NextResponse.json({ success: false, error: "Apenas admin pode configurar URL pública" }, { status: 403 });
  }
  return null;
}
...
export async function GET(req: NextRequest) {
  // Qualquer logado pode LER a URL pública — é info do sistema (não segredo) e o
  // badge "Webhook · Online" no topo do painel precisa pra todos os clientes.
  // Só o detect=true (auto-scan ngrok local) e o POST exigem admin.
  const ctx = await requireClientId(req);
  if (!ctx.ok) return ctx.response;
  const { searchParams } = new URL(req.url);
  const detect = searchParams.get("detect");

  // MODO: Auto-detectar via Ngrok local API — só admin + só em dev
  if (detect === "true") {
    if (!ctx.isAdmin) {
      return NextResponse.json({ success: false, error: "Apenas admin pode auto-detectar ngrok" }, { status: 403 });
    }
    if (process.env.NODE_ENV === "production") {
      return NextResponse.json({ success: false, error: "Auto-detect Ngrok só funciona em dev local." }, { status: 400 });
    }
    try {
