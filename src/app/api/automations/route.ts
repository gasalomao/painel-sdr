...
export async function POST(req: NextRequest) {
  try {
    ...
    // SaaS guard: cliente não escolhe modelo. Sobrescreve dispatch_ai_model
    // e followup_ai_model do body pelo client.default_ai_model. Admin livre.
    await enforceClientDefaultModel(body, ctx, ["dispatch_ai_model", "followup_ai_model"]);
    if (!body?.name?.trim()) {
    ...
    if (!(await isInstanceOwnedByClient(String(body.instance_name), ctx.clientId))) {
    ...
    const row = {
      ...
      dispatch_humanize: !!body.dispatch_humanize,
      dispatch_ai_model: body.dispatch_ai_model || null,
      dispatch_ai_prompt: body.dispatch_ai_prompt || null,
      dispatch_media_url: body.dispatch_media_url || null,
      dispatch_media_type: body.dispatch_media_type || null,
      dispatch_media_caption: body.dispatch_media_caption || null,
      dispatch_media_file_name: body.dispatch_media_file_name || null,
      dispatch_media_mimetype: body.dispatch_media_mimetype || null,
