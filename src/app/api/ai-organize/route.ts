...
export async function POST(req: NextRequest) {
  ...
  // Sem clientId = modo legado/global (apenas admin manual).
  let clientIdScope: string | null = null;

  // AUTH: aceita
  //  (a) header secret interno (scheduler + run-now server-side), OU
  //  (b) cookie de sessão. Se cookie de cliente comum → clientId TEM que ser o
  //      próprio. Admin pode passar qualquer clientId ou nenhum.
  ...
  if (!isInternal) {
    ...
    sessionClientId = ctx.clientId;
    sessionIsAdmin = ctx.isAdmin;
  }
  ...
  try {
    ...
    // OWNERSHIP: cliente comum só dispara pro próprio escopo. Admin pode tudo.
    if (!isInternal && !sessionIsAdmin) {
      if (clientIdScope && clientIdScope !== sessionClientId) {
        return NextResponse.json(
          { success: false, error: "clientId do body não corresponde à sessão" },
          { status: 403 }
        );
      }
    ...
    // Chave OpenRouter (provedor alternativo). Sempre lida da config central.
    let openrouterApiKey: string | null = null;
    if (!apiKey || !model || !provider) {
      const { data: cfg, error: cfgErr } = await adminClient
        .from("ai_organizer_config")
        .select("api_key, openrouter_api_key, model, provider")
        .eq("id", 1)
        .maybeSingle();
      ...
      if (cfg) {
        ...
        openrouterApiKey = (cfg as any).openrouter_api_key || null;
      }
    }
    if (!openrouterApiKey) {
      const { data: orCfg } = await adminClient
        .from("ai_organizer_config").select("openrouter_api_key").eq("id", 1).maybeSingle();
      openrouterApiKey = (orCfg as any)?.openrouter_api_key || null;
    }

    // Se o modelo escolhido é um ref do OpenRouter ("openrouter:..."), o provedor
    // efetivo é OpenRouter — independente do campo legado `provider`.
    const { providerOf } = await import("@/lib/ai-provider");
    const refProvider = providerOf(model);
    if (refProvider === "openrouter") provider = "OpenRouter";

    const needsKey = provider === "OpenRouter" ? openrouterApiKey : apiKey;
    if (!needsKey) {
    ...
    // + APPOINTMENTS estruturados (ground truth do calendário do agente).

    // Puxa agendamentos confirmed/completed dos JIDs sendo analisados pra que
    // R15/R17 deixem de inferir da conversa e passem a usar fonte estrutural.
    // Confirmed no futuro → IA mantém em "agendado". Completed/passado +
    // agradecimento → IA sobe pra terminal positivo. Reduz erro de organizer
    // por ~30% em conta de salão/clínica (cliente recorrente).
    const appointmentsByJid = new Map<string, Array<{ start_at: string; end_at: string; status: string; title?: string; service_name?: string | null }>>();
    try {
      let apptQ = adminClient
        .from("appointments")
        .select("remote_jid, start_at, end_at, status, title, service_name")
        .in("remote_jid", numerosProcessados)
        .in("status", ["confirmed", "completed", "no_show"]);
      if (clientIdScope) apptQ = apptQ.eq("client_id", clientIdScope);
      const { data: apptRows } = await apptQ;
      for (const row of apptRows || []) {
        if (!appointmentsByJid.has(row.remote_jid)) appointmentsByJid.set(row.remote_jid, []);
        appointmentsByJid.get(row.remote_jid)!.push({
      ...
      console.warn("[AI-ORGANIZE] não consegui ler appointments (não-fatal):", (e as Error).message);
    }

    // Achata todos os appointments num único array — o organizer recebe
    // contexto agregado e o prompt já instrui a IA a relacionar com cada JID
    // via o bloco "CHAT ID:" das threads. Em conta com volume alto, isso
    // ainda fica leve (raramente >50 appointments ativos por tick).
    const allAppts = Array.from(appointmentsByJid.values()).flat();
    ...
    if (skipAi) {
      console.log("[AI-ORGANIZE] Nenhum chat ambíguo. Pulando chamada IA.");
        ...
        // Roteia pela camada unificada (OpenAI-compatible) com saída JSON.
        const { generateText } = await import("@/lib/ai-provider");
        try {
          const out = await generateText({
            modelRef: model,
            system: systemInstruction,
            prompt: `CONVERSAS A ANALISAR:\n\n${threadsTexto}`,
            temperature: 0.2,
            jsonMode: true,
            openrouterApiKey,
          });
          ...
          throw new Error(`Erro na API OpenRouter: ${err?.message || String(err)}`);
        }
    }
    ...
    let alteracoes = 0;
    const logs = [];
    const upsertsBatch: any[] = []; // F: batch upsert em vez de N+1 UPDATEs
    ...
    for (const item of resultadosLista) {
       ...
       // Antes: UPDATE/INSERT sequencial por lead (N+1, 50-100ms cada → 200 leads = 10-20s).
       // Agora: monta em memória e faz upsert em batch logo após o loop.
       if (isLeadNovo) {
           upsertsBatch.push({
               remoteJid: jid,
               status: novoStatus,
    ...
    // F — BATCH UPSERT: aplica todas as mudanças em uma chamada por chunk.
    // Antes eram 200 UPDATEs sequenciais (10-20s); agora 1-2 chamadas (~300ms).
    if (upsertsBatch.length > 0) {
      const CHUNK = 100;
      for (let i = 0; i < upsertsBatch.length; i += CHUNK) {
