...
async function requireAdmin(req: NextRequest) {
  ...
  if (!ctx.isAdmin) {
    return NextResponse.json({ success: false, error: "Apenas admin pode mexer nas credenciais da Evolution" }, { status: 403 });
  }
  return null;
}
...
async function probe(url: string, apiKey: string): Promise<{ ok: boolean; instances?: any[]; error?: string; status?: number; }> {
  try {
    // SSRF guard: em produção, recusa probe contra hosts privados/localhost.
    if (process.env.NODE_ENV === "production") {
      const { isPrivateOrLocalUrl } = await import("@/lib/url-guard");
      if (isPrivateOrLocalUrl(url)) {
        return { ok: false, error: "URL aponta pra rede privada/localhost — não permitido em produção." };
      }
    }
