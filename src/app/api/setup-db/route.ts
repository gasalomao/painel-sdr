...
/**
 * Guard: TODAS as operações de setup-db são EXCLUSIVAS de admin.
 * Antes era público — qualquer um sobrescrevia .env.local.
 */
async function requireAdmin(req: NextRequest) {
  const ctx = await requireClientId(req);
  if (!ctx.ok) return ctx.response;
  if (!ctx.isAdmin) {
    return NextResponse.json({ success: false, error: "Apenas admin pode acessar setup-db" }, { status: 403 });
  }
  return null;
}
...
export async function POST(req: NextRequest) {
  const denied = await requireAdmin(req);
  if (denied) return denied;
  // Em produção .env.local é gerenciado pelo painel de deploy (Easypanel) — não pode
  // ser sobrescrito em runtime. Bloqueio defensivo contra admin malicioso ou XSS.
  if (process.env.NODE_ENV === "production") {
    return NextResponse.json(
      { success: false, error: "Em produção a config Supabase é definida nas Environment Variables do deploy, não via API." },
      { status: 403 }
    );
  }
  try {
