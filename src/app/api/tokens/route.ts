...
export async function GET(req: NextRequest) {
  try {
    ...
    // Breakdown POR CLIENTE — apenas admin (cliente comum só vê o próprio
    // agregado, não tem essa visão). Faz lookup batch de clients.name pros
    // UUIDs encontrados pra UI mostrar "Padaria do João: R$ 42,15" em vez
    // de "00000000-...-001: R$ 42,15".
    let byClient: any[] | undefined = undefined;
    if (ctx.isAdmin) {
      const map: Record<string, { total: number; cost: number; calls: number; prompt: number; completion: number }> = {};
      for (const r of rows) {
        const k = r.client_id || "(sem cliente)";
        if (!map[k]) map[k] = { total: 0, cost: 0, calls: 0, prompt: 0, completion: 0 };
        map[k].total      += Number(r.total_tokens || 0);
        map[k].cost       += r._cost_live;
        map[k].calls      += 1;
      ...
      if (clientIds.length > 0 && supabaseAdmin) {
        ...
        for (const c of (clients || []) as any[]) {
          nameMap[c.id] = c.name || c.email || c.id;
        }
      }
      byClient = Object.entries(map)
        .map(([id, v]) => ({
          client_id: id === "(sem cliente)" ? null : id,
          name: id === "(sem cliente)" ? "(sem cliente)" : (nameMap[id] || id),
          ...v,
          costBrl: v.cost * brlRate,
        }))
        .sort((a, b) => b.cost - a.cost || b.total - a.total);
    }
