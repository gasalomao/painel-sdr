...
export default function TokensPage() {
  ...
  // Breakdown POR CLIENTE — só admin recebe esses dados do /api/tokens.
  // Cliente comum: byClient vem undefined → bloco da UI fica oculto.
  const [byClient, setByClient] = useState<Array<{
    client_id: string | null; name: string; total: number; cost: number;
    costBrl: number; calls: number; prompt: number; completion: number;
  }> | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (d.success) {
        ...
        // Admin-only: backend só envia byClient quando isAdminView=true
        setByClient(d.byClient || null);
        setIsAdminView(!!d.isAdminView);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-[100dvh] max-h-[100dvh] bg-background overflow-hidden text-white">
      ...
      <main className="flex-1 overflow-y-auto p-3 sm:p-6 md:p-10 max-w-7xl mx-auto w-full space-y-4 sm:space-y-6 mobile-safe-bottom">
        ...
        {/* Breakdown POR CLIENTE — admin-only. Mostra qual tenant gastou
            quanto. Cliente comum não recebe byClient da API, então isso
            simplesmente não renderiza pra ele. */}
        {isAdminView && byClient && byClient.length > 0 && (
        <Card className="border-purple-500/30 bg-purple-500/[0.03]">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-black uppercase tracking-widest flex items-center gap-2 text-purple-200">
              <ListChecks className="w-4 h-4 text-purple-400" /> Custos por cliente
              <span className="text-[9px] font-normal text-purple-300/70 normal-case tracking-normal ml-auto">
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {byClient.map(c => (
                    <tr key={c.client_id || "_none"} className="border-b border-white/[0.03]">
                      <td className="p-2">
                        <p className="text-white font-bold">{c.name}</p>
                        {c.client_id && (
                          <p className="text-[9px] font-mono text-muted-foreground/60 truncate max-w-[280px]">{c.client_id}</p>
                        )}
                      </td>
                      <td className="p-2 text-right font-mono">{c.calls}</td>
                      <td className="p-2 text-right font-mono text-amber-300">{formatTokens(c.total)}</td>
                      <td className="p-2 text-right font-mono text-emerald-300">${c.cost.toFixed(4)}</td>
                      <td className="p-2 text-right font-black text-emerald-200">{formatBRL(c.costBrl)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-purple-500/30 bg-purple-500/[0.05]">
                    <td className="p-2 text-[10px] uppercase tracking-widest font-black text-purple-300">Total</td>
                    <td className="p-2 text-right font-mono font-black text-purple-100">{byClient.reduce((a, c) => a + c.calls, 0)}</td>
                    <td className="p-2 text-right font-mono font-black text-amber-200">{formatTokens(byClient.reduce((a, c) => a + c.total, 0))}</td>
                    <td className="p-2 text-right font-mono font-black text-emerald-200">${byClient.reduce((a, c) => a + c.cost, 0).toFixed(4)}</td>
                    <td className="p-2 text-right font-black text-emerald-100">{formatBRL(byClient.reduce((a, c) => a + c.costBrl, 0))}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
