...
export async function analyzeLead(opts: {
  leadId: number;
  /** Chave Gemini (fallback). Pode ser null se o modelo for OpenRouter. */
  apiKey?: string | null;
  model?: string;
  force?: boolean;
  ...
  const { data: lead, error } = await supabaseAdmin
    ...
    .select("id, client_id, \"remoteJid\", nome_negocio, ramo_negocio, categoria, endereco, website, avaliacao, reviews, intelligence, intelligence_at, icp_score, lead_type")
    .eq("id", opts.leadId)
    .maybeSingle();
  ...
  const model = (await resolveModel(opts.model, opts.clientId || lead.client_id)) || "gemini-2.5-flash"; // último resort se admin nunca configurou nada
  const { generateText, providerOf, providerDisplayName } = await import("@/lib/ai-provider");
  const { getAiKeys } = await import("@/lib/ai-keys");
  const keys = await getAiKeys();
  const geminiApiKey = opts.apiKey || keys.gemini;
  const openrouterApiKey = keys.openrouter;
  const provider = providerOf(model);
  const providerLabel = providerDisplayName(provider);
  // thinking nativo só no Gemini 2.5/3 (dynamic = -1). OpenRouter ignora.
  const supportsNativeThinking = provider === "gemini" && /gemini-(2\.5|3)/i.test(model);
  const thinkingBudget: number | null | undefined = supportsNativeThinking ? -1 : undefined;
  ...
  try {
    const thinkOut = await generateText({
      modelRef: model,
      system: `${sellerContext}\n\n${THINK_SYSTEM_PROMPT}`,
  ...
  const jsonOut = await generateText({
    modelRef: model,
    system: `${sellerContext}\n\n${SYSTEM_PROMPT}`,
    prompt: userPrompt,
    temperature: 0.2,
