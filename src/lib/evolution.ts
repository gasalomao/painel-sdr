...
async function evoFetch(path: string, method: string = "GET", body?: unknown, timeoutMs: number = 30000) {
  await loadEvoCfg();
  ...
  try {
    const res = await axios({
      ...
      data: body,
      timeout: timeoutMs, // 30s default; presence usa curto (cosmético, não pode travar envio)
      httpsAgent,
...
export const evolution = {
  ...
  async sendPresence(number: string, presence: "composing" | "recording" | "paused", instance?: string) {
    instance = await resolveInstance(instance).catch(() => "");
    if (!instance) return null;
    // Timeout curto: o "digitando" é cosmético. Se a instância estiver lenta/caída,
    // não pode segurar o envio real por 30s — falha rápido (6s) e segue.
    return evoFetch(`/chat/presence/${instance}`, "POST", {
      number,
      presence
    }, 6000).catch(() => null); // Ignora erros de presença para não travar o envio
  },
  ...
  /**
   * Registra webhook na Evolution.
   *
   * Se `secret` for passado, manda também `headers: { X-Webhook-Secret: <secret> }`
   * no payload — Evolution v2 (versões recentes) passa esse header em cada
   * webhook que dispara, e nosso `/api/webhooks/whatsapp` valida contra o
   * mesmo secret salvo em `channel_connections.provider_config.webhook_secret`.
   *
   * Versões antigas da Evolution ignoram o campo `headers` — nesse caso o
   * webhook continua funcionando (sem header) e nosso handler aceita em
   * modo backwards-compat (warning em prod).
   */
  async setWebhook(url: string, instance?: string, secret?: string | null) {
    instance = await resolveInstance(instance);
    const payload: any = {
      webhook: {
        url,
    ...
    if (secret) {
      payload.webhook.headers = { "X-Webhook-Secret": secret };
    }
    return evoFetch(`/webhook/set/${instance}`, "POST", payload);
  },
  ...
  /**
   * Igual ao checkWhatsAppNumbers, mas devolve TAMBÉM o `jid` canônico que o
   * WhatsApp realmente usa pra cada número. Crítico no Brasil: o número
   * captado pode ter o 9º dígito (ex: 5571999999999) mas o JID real do
   * WhatsApp ser sem ele (5571999999999 → 557199999999). Disparar/salvar
   * com o JID canônico mantém a conversa do disparo e a resposta do cliente
   * NO MESMO chat.
   */
  async checkWhatsAppNumbersDetailed(
    numbers: string[],
    instance?: string
    ...
    try {
      ...
      for (const item of list) {
        const key = (item.number || item.jid || "").replace(/\D/g, "");
        if (key) map[key] = { exists: !!item.exists, jid: item.jid || null };
      }
      return map;
    } catch {
      return {};
