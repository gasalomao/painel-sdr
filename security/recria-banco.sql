-- =====================================================================
-- RECRIACAO COMPLETA DO BANCO painel-sdr (DDL oficial do app)
-- Rodar num banco NOVO/VAZIO via SQL Editor. Idempotente (IF NOT EXISTS).
-- Depois: restaurar os DADOS do schema backup_full (security/backup-restore.sql)
-- ou do pg_dump (bloco 5).
...
CREATE TABLE IF NOT EXISTS public.automations (
  ...
  dispatch_humanize          boolean DEFAULT false,
...
-- =====================================================================
-- BLINDAGEM EXTRA (defesa em profundidade — além dos REVOKEs acima)
-- =====================================================================
-- 1) RLS habilitado SEM políticas nas 7 tabelas sensíveis: mesmo que um
--    GRANT acidental aconteça no futuro, anon/authenticated continuam
--    sem acesso (RLS sem policy = deny all). service_role BYPASSA RLS,
--    então o servidor do painel não é afetado.
...
-- 2) Ninguém além do dono/service_role cria objetos no schema public.
REVOKE CREATE ON SCHEMA public FROM PUBLIC, anon, authenticated;

-- 3) Sem funções expostas ao anon (PostgREST RPC) sem necessidade.
REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM anon, authenticated;

-- 4) Grants futuros seguros: tabelas criadas depois seguem negadas ao anon
--    por padrão (o setup do app concede só o que precisa).
