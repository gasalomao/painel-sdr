/**
 * Migração V2: Criação das tabelas normalizadas (contacts, sessions, messages)
 * Executa via Supabase Admin Client (service_role_key)
 */
...
async function runSQL(sql, label) {
  ...
  if (error) {
    // Fallback: tenta via REST se a função RPC não existir
    if (error.message.includes('exec_sql')) {
      console.log(`   ⚠️ RPC não disponível. Executar manualmente no Supabase Dashboard.`);
      return false;
    }
...
async function migrate() {
  console.log("==============================================");
  console.log("  MIGRAÇÃO V2 — SISTEMA OMNICHANNEL");
  console.log("==============================================");

  // Teste de conexão
  const { data, error } = await supabase.from('agent_settings').select('id').limit(1);
  if (error) {
  ...
  // Vamos tentar criar tabelas diretamente via insert/upsert approach
  // Como o Supabase não permite DDL via client, vamos gerar o SQL para o dashboard
...
SELECT 'Migração concluída com sucesso!' AS status;
  ...
  // Salvar o SQL em um arquivo para execução manual
  const fs = require('fs');
  const sqlPath = path.resolve(__dirname, 'migrate_v2.sql');
  fs.writeFileSync(sqlPath, migrationSQL, 'utf-8');
  console.log(`\n📄 SQL de migração salvo em: ${sqlPath}`);
  console.log("\n⚠️  IMPORTANTE: Copie e execute este SQL no Supabase Dashboard > SQL Editor");
  console.log("   URL: https://supabase.com/dashboard → Seu Projeto → SQL Editor → New Query");
  console.log("\n   (O Supabase Client não permite DDL/CREATE TABLE via API)");
}
